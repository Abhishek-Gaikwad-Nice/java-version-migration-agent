...
#### When Local Dependency Tree Cannot Be Run (MANDATORY — do NOT defer to CI)

If `./gradlew dependencies` cannot be executed locally (e.g., missing CodeArtifact/Artifactory auth token, no network access to private registry), **do not defer BOM alignment to a PR action item**. A deferred BOM split will cause a `ClassNotFoundException` at container startup — a runtime failure that could have been caught here.

Instead, look up the transitive dependencies of every upgraded library directly on Maven Central **before committing**:

1. For each library flagged as UPGRADE REQUIRED in Phase 2, fetch its POM:
   ```
   https://repo1.maven.org/maven2/{groupId-slash-separated}/{artifactId}/{version}/{artifactId}-{version}.pom
   # Example — micrometer-registry-cloudwatch2:1.15.4:
   https://repo1.maven.org/maven2/io/micrometer/micrometer-registry-cloudwatch2/1.15.4/micrometer-registry-cloudwatch2-1.15.4.pom
   ```
2. In the POM, find the `<dependencies>` block and identify any dependency belonging to an SDK family that has a BOM pinned in the project (e.g., `software.amazon.awssdk`, `io.micrometer`, `org.springframework`).
3. If the version in the POM is **higher** than the current BOM pin in the project — upgrade the BOM to that version now, in this phase, before committing.

```
❌ BAD: Leave a PR action item "check AWS SDK BOM alignment after CI passes"
✅ GOOD: Fetch the POM, find software.amazon.awssdk:cloudwatch:2.31.78, upgrade bom:2.20.153 → 2.31.78 now
```

**Real example**: `micrometer-registry-cloudwatch2:1.15.4` declares `software.amazon.awssdk:cloudwatch:2.31.78` in its POM. A project pinning `software.amazon.awssdk:bom:2.20.153` must therefore upgrade the BOM to `2.31.78` — otherwise `sts-2.20.153.jar` tries to call into `aws-core-2.31.78.jar` and fails with `ClassNotFoundException: QueryParametersToBodyInterceptor` at startup.
...
### Pitfall 7 — BOM Split Version Missed Because Local Build Was Blocked
**Symptom**: CI passes, but the container crashes at startup with `ClassNotFoundException` or `NoClassDefFoundError` (e.g., `software.amazon.awssdk.protocols.query.interceptor.QueryParametersToBodyInterceptor`).
**Cause**: A library was upgraded (e.g., `micrometer-registry-cloudwatch2:1.11.5 → 1.15.4`) which transitively pulls a **newer version** of an SDK family (e.g., `aws-core:2.31.78`). The project's BOM pin for that family (`software.amazon.awssdk:bom:2.20.153`) was not upgraded. Because the local dependency tree was skipped (no auth token for the private registry), the split went undetected until runtime in the container.
**Prevention**: Even when `./gradlew dependencies` cannot run locally, fetch the POM of every upgraded library from Maven Central and check for transitive SDK version bumps. Upgrade the BOM to align before committing. See "When Local Dependency Tree Cannot Be Run" in Phase 4.
**Fix if already hit**: The jar filename in the stack trace reveals the actual resolved versions (e.g., `sts-2.20.153.jar` vs `aws-core-2.31.78.jar`). Upgrade the BOM to the highest version shown in the stack trace.
